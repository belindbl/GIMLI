#Author : Fawaz. the logic here is: lidar1 > offset time delay in ms > cam 1
#I tried many ways, but they all fail. so the solution is unreal time. 
#I will write my experiments in details later.
...
# Offset time between LIDAR and Camera captures (in milliseconds)
OFFSET_TIME_MS = 4500  # Set this to the desired offset time
...
def SerializeToPCLFileContent(numPoints, posX, posY, posZ, quatW, quatX, quatY, quatZ, point_array):
    ...
    for p in point_array:
        ...
        if not math.isinf(p[3]) and p[3] != 0:
            intensity = int(p[3])
        pclFileContent += '%.5f %.5f %.5f %d\n' % (p[0], -p[1], p[2], intensity) # Inverted Y coordinate to match Open3D's coordinate system
...
def lidar_thread():
    ...
    while lidar_pcd_count < MAX_LIDAR_PCDS and not stop_threads_event.is_set():  # Stop
        ...
        # Wait for the offset time before signaling the camera to capture
        time.sleep(OFFSET_TIME_MS / 1000.0)
