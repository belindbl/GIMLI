...
def crop_and_filter(image):
    # Convert the PIL image to a PyTorch tensor
    image_tensor = T.ToTensor()(image).unsqueeze(0).cuda()  # Add batch dimension and move to GPU
    
    # Get image dimensions (height, width, channels)
    _, height, width = image_tensor.shape

    # Crop the image horizontally by 30% from the bottom
    new_height = int(height * 0.7)  # 70% of the original height
    cropped_image = image_tensor[:, :new_height, :]

    # Apply brightening filter (increase brightness)
    enhancer = 2.5  # Factor for brightness
    brightened_image = cropped_image * enhancer

    # Ensure pixel values are within valid range [0, 1]
    brightened_image = torch.clamp(brightened_image, 0, 1)

    # Convert the tensor back to a PIL image after processing
    brightened_image = brightened_image.squeeze().cpu().numpy().transpose(1, 2, 0)  # Remove batch and move to CPU
    brightened_image = Image.fromarray((brightened_image * 255).astype(np.uint8))  # Convert back to PIL image
...
def process_frame(image, model):
    ...
    return cv2.cvtColor(processed_image, cv2.COLOR_RGB2RGBA)
...
def yolo_inference_thread():
    ...
    try:
        ...
        _ = model(dummy_image)  # Force initialization
        model.conf = 0.4
        model.iou = 0.5
    ...
    while not stop_threads_event.is_set():
        ...
        if cam_data:
            try:
                ...
                # Crop and filter the image before pushing to the queue
                pil_img = Image.fromarray(img)  # Convert numpy array to PIL Image
                processed_img = crop_and_filter(pil_img)
                img = np.array(processed_img)  # Convert back to numpy array if needed
...
def setup_visualizer():
    ...
    vis.create_window(width=600, height=400, window_name="PCD Visualisation")
    render_option = vis.get_render_option()
    render_option.background_color = np.asarray([0.8, 0.8, 0.8])  # Light gray
    render_option.point_size = 2

    point_cloud = o3d.geometry.PointCloud()
    point_cloud.paint_uniform_color([0, 0, 0.5])  # Dark blue initial color
...
def lidar_processing_thread():
    ...
    while not stop_threads_event.is_set():
        ...
        if lidar_data:
            try:
                # Decode LiDAR data into point cloud (inverting Y axis)
                sizeofFloat = 4
                header_count = 11
                ...
                points = np.stack([point_array[:, 0], point_array[:, 1], point_array[:, 2]], axis=1) # No longer inverting Y-coord.

                # Filter out boat points
                mask, filtered_points = filter_boat_points(points, min_coords, max_coords)

                # Run DBSCAN on filtered points
                ...
                # Update persistent IDs for clusters
                ...
                # Update view controls and render
                ctr.set_lookat([0, 0, 300])
                ctr.set_zoom(0.02)
                ctr.set_front([0, 0, -1])
                ctr.set_up([1, 0, 0])
...
if __name__ == "__main__":
    ...
    # Retrieve sensor list via TCP and extract camera parameters
